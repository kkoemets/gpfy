...
const lookIntoBitcoinUrl = "https://www.lookintobitcoin.com/charts/bitcoin-investor-tool/";
...
const retrieveScreenshot: () => Promise<void> = async () => {
  ...
  if (doesScreenshotExistAlready) {
    if (isFileFromTheSameDay((await fs.promises.stat(graphScreenshotPath)).birthtime)) {
